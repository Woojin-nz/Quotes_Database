...
if(isset($_SESSION['admin'])) {
    ...
    $deleteauthor_sql= "SELECT * FROM `authors` WHERE `Author_ID`=".$_REQUEST['ID'];
    $deleteauthor_query=mysqli_query($dbconnect,$deleteauthor_sql);
    $deleteauthor_rs = mysqli_fetch_assoc($deleteauthor_query);

    $full_name = $deleteauthor_rs['First']." ".$deleteauthor_rs['Initial']." ".$deleteauthor_rs['Last'];
...
<div class="error">
    There are <?php echo $author_quotes_count; ?> quotes associated with <?php echo $full_name ?>
    and those will also be removed from the database.
</div>
